...
ax.set_title('units:mm/day',fontsize=14, loc='right',pad=12)
 #保存图片
 fig.savefig('v_hm-filter.png',format='png',dpi=150)
```
...
ax.yaxis.set_ticklabels([]) # 坐标轴的标签不显示
ax.set_xticks([]) #坐标轴消失，啥也没有
ax4.spines[['right','left','top','bottom']].set_linewidth(1) # 设置四个边框轴的多个边，加粗
cb.outline.set_linewidth(0.8)  # 设置colorbar色条外面的框的粗细
ax.spines[['right','left','top','bottom']].set_linewidth(1.5) 
ax.spines[['right','left','top','bottom']].set_visible(True)   # 这样在有投影的情况下，改粗细才有效果
...
ax.set_yscale("symlog")  #设置y轴坐标系的缩放比例为高度层的形式
